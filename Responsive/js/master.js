/* Index:
 ...
 4.8 Get height of the Content landing page Key, boxes and apply the use the height of heighest
 4.10 Cookies message
 4.11 Odd and Even class added on Case studies, News listing
 */
...
//4.6 Equalising feeds height on desktops
...
window.addEventListener("hashchange",function() {
    ...
    if (element.length) {
      ...
      jQuery(idFromHash).addClass('active');
        if (window.location.href.indexOf("#tab_")> -1) {
          window.scroll({
            top: 0,
            left: 0,
            behavior: 'smooth'
...
//4.8 Get height of the Content campaign landing page Key, boxes and apply the use the height of heighest
jQuery(document).ready(function() {
  if (jQuery('.campaign_landing_page').length > 0)  {
    jQuery('.campaign_landing_page .row3').each(function(){
         var boxes = jQuery('.col-3',this);
         var maxHeight = Math.max.apply(Math, boxes.map(function(){
             return jQuery(this).height();
         }).get());
         jQuery (boxes.height(maxHeight));
    });
  }
});
...
setTimeout(function(){
    jQuery(document).ready(function() {
      if (jQuery('.campaign_landing_page').length > 0)  {
        jQuery('.campaign_landing_page .holder').each(function(){
             var pichold = jQuery('.picholder',this);
             var maxHeight = Math.max.apply(Math, pichold.map(function(){
                 return jQuery(this).height();
             }).get());
             jQuery (pichold.height(maxHeight));
        });
      }
    });
}, 1500);
...
//4.10 Cookies pop up Session Storage     
jQuery(document).ready(function(){
    if (sessionStorage.getItem('visitOnce') !== 'true') {
      jQuery('body').append('<div id="cookieWrapper"><div id="cookieNotice"><p>We use cookies for analytics, personalised content and ads. By continuing to browse this site, you agree to this use. <a class="cookieButton" >Close</a><a href="http://www.lsbu.ac.uk/footer/privacy">Privacy Policy</a></p></div></div>').fadeIn(1000);
      sessionStorage.setItem('visitOnce', 'true');
    }
    
    jQuery('#cookieNotice, .cookieButton').on('click', function() {
      jQuery('#cookieNotice').fadeOut(300);
      sessionStorage.setItem('visitOnce', true);
    });
});
...
//4.11 Odd and Even class added on Case studies, News listing
jQuery(document).ready( function () {
  jQuery('#tab_case_studies').each(function(){
    jQuery('.media:odd').addClass('odd');
    jQuery('.media:even').addClass('even');
   });
 });
